...
local function LogSuspicious(source, reason)
    ...
    print(string.format("Player: %s (%s) | Reason: %s", playerName, identifier, reason))
...
local function HandleScopeItemUse(source, itemName, weaponType, scopeType)
    ...
    TriggerClientEvent("scopes:client:AttachScope", source, weaponType, scopeType)
    ...
    if Config.Debug then
        print(string.format("Player %s used scope item: %s", GetPlayerName(source), itemName))
    end
end
...
RegisterNetEvent("scopes:server:ScopeAttached", function(weaponType, scopeType)
    local source = source
    
    if Config.Debug then
        print(string.format("Scope attached confirmation from player %s", GetPlayerName(source)))
    end
...
RegisterNetEvent("scopes:server:ReturnScopeItem", function(scopeItem)
    local source = source
    ...
    if Framework.AddItem(source, scopeItem, 1) then
        if Config.Debug then
            print(string.format("Returned scope item %s to player %s", scopeItem, GetPlayerName(source)))
        end
    else
...
Citizen.CreateThread(function()
    ...
    for weaponType, itemName in pairs(Config.Items.scopes) do
        ...
        if Config.Debug then
            print(string.format("Registered usable item: %s (%s scope)", itemName, scopeType))
        end
    end
    ...
    if Config.General.requireToolForRemoval then
        Framework.RegisterUsableItem(Config.Items.attachTool, function(data)
            ...
            TriggerClientEvent("scopes:client:RemoveScope", source)
        ...
        if Config.Debug then
            print(string.format("Registered tool for scope removal: %s", Config.Items.attachTool))
        end
    end
end)
...
if Config.General.adminCommand then
    ...
    RegisterCommand(Config.Commands.adminAddScope, function(source, args, rawCommand)
        ...
        TriggerClientEvent("scopes:client:AdminAttachScope", source)
    ...
    RegisterCommand(Config.Commands.adminRemoveScope, function(source, args, rawCommand)
        ...
        TriggerClientEvent("scopes:client:AdminRemoveScope", source)
...
Citizen.CreateThread(function()
    ...
    print(string.format("Framework: %s | Items: %s | Commands: %s", 
        Framework.Type, 
...
if Config.Debug then
    print("Server script loaded in debug mode")
end
